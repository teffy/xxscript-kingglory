...
points = coordinate_data.points
rangecolors = coordinate_data.rangecolors
...
--下一步
click(points.challengeNextStep, 200)
...
while true do
	...
	findThen(
		rangecolors.challengeDetailNoHero.range,
		rangecolors.challengeDetailNoHero.color,
		function(x, y)
			if x > -1 then
				sysLog("没选够3个英雄")
				--点击更换阵容
				...
				findThen(
					rangecolors.pickHeroNoHero3.range,
					rangecolors.pickHeroNoHero3.color,
					function(x, y)
						if x > -1 then
							--选3个英雄
	...
	--加载页面
	findThen(
		rangecolors.loading100.range,
		rangecolors.loading100.color,
		function(x, y)
			if x > -1 then
				sysLog("加载即将结束")
				return true
	...
	findThen(
		rangecolors.monvIsInFighting.range,
		rangecolors.monvIsInFighting.color,
		function(x, y)
			if x > -1 then
				findThen(
					rangecolors.monvIsAuto.range,
					rangecolors.monvIsAuto.color,
					function(x, y)
						if x > -1 then
							sysLog("已经开启自动")
						else
	...
	findThen(
		rangecolors.monvInFightingSkip.range,
		rangecolors.monvInFightingSkip.color,
		function(x, y)
			if x > -1 then
	...
	while true do
		...
		successP1 =
			findColors(rangecolors.fightResultSuccessPoint1.range, rangecolors.fightResultSuccessPoint1.color, 95, 0, 0, 0)
		successP2 =
			findColors(rangecolors.fightResultSuccessPoint2.range, rangecolors.fightResultSuccessPoint2.color, 95, 0, 0, 0)
		successP3 =
			findColors(rangecolors.fightResultSuccessPoint3.range, rangecolors.fightResultSuccessPoint3.color, 95, 0, 0, 0)
		successP4 =
			findColors(rangecolors.fightResultSuccessPoint4.range, rangecolors.fightResultSuccessPoint4.color, 95, 0, 0, 0)
		if (#successP1 ~= 0 or #successP2 ~= 0 or #successP3 ~= 0) and #successP4 ~= 0 then
			...
			failedP1 =
				findColors(rangecolors.fightResultFailedPoint1.range, rangecolors.fightResultFailedPoint1.color, 95, 0, 0, 0)
			failedP2 =
				findColors(rangecolors.fightResultFailedPoint2.range, rangecolors.fightResultFailedPoint2.color, 95, 0, 0, 0)
			failedP3 =
				findColors(rangecolors.fightResultFailedPoint3.range, rangecolors.fightResultFailedPoint3.color, 95, 0, 0, 0)
	...
	if fightResult then
		...
		while true do
			...
			resultSuccessFightAgain =
				findColors(rangecolors.resultSuccessFightAgain.range, rangecolors.resultSuccessFightAgain.color, 95, 0, 0, 0)
			if #resultSuccessBack ~= 0 and #resultSuccessFightAgain ~= 0 then
				findThen(
					rangecolors.resultSuccessGoldLimit.range,
					rangecolors.resultSuccessGoldLimit.color,
					function(x, y)
						if x > -1 then
	...
	sysLog("fightAllTime:" .. fightAllTime .. ",fightCount:" .. fightCount .. ",averageTime:" .. averageTime)
end
